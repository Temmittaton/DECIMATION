function scarab_choose(){
	if (place_meeting(x, y, obj_player) && state != scarab_upsideDown && state != scarab_hurt) {
		obj_player.dmg = id;
		return mob_stun;
	}
	...
	else if (state == scarab_hurt && place_meeting (x, y, obj_player)) {
		return scarab_upsideDown;
	}
	else if ((state == scarab_hurt) && (animation_end())) {
		return scarab_upsideDown;
	}
}